...
fury_tx iid create-iid-from-legacy-did "$MIGUEL_DID_FULL"
echo "Ledgering DID 2/3..."
fury_tx iid create-iid-from-legacy-did "$FRANCESCO_DID_FULL"
echo "Ledgering DID 3/3..."
fury_tx iid create-iid-from-legacy-did "$SHAUN_DID_FULL"
...
fury_q bonds current-reserve "$BOND_DID"
echo "Available reserve..."
fury_q bonds available-reserve "$BOND_DID"
echo "Withdrawing 400000res using the controller DID..."
fury_tx bonds withdraw-reserve "$BOND_DID" 400000res "$FRANCESCO_DID_FULL"
echo "Reserve withdraw account..."
fury_q bank balances "$RESERVE_OUT"
echo "Current reserve..."
fury_q bonds current-reserve "$BOND_DID"
echo "Available reserve..."
fury_q bonds available-reserve "$BOND_DID"
...
fury_tx bonds withdraw-reserve "$BOND_DID" 5000res "$FRANCESCO_DID_FULL"
echo "Cannot withdraw 595001res due to insufficient funds..."
fury_tx bonds withdraw-reserve "$BOND_DID" 595001res "$FRANCESCO_DID_FULL"
echo "Reserve withdraw account..."
fury_q bank balances "$RESERVE_OUT"
echo "Current reserve..."
fury_q bonds current-reserve "$BOND_DID"
